...
- 初回読み込み中の起動画面表示
- オフライン起動対応
- バックグラウンドで新しいバージョンを検知した際の更新バナー表示
- PWA対応
...
HTML・CSS・JavaScript・アイコンはService Workerのアプリシェルキャッシュへ保存され、2回目以降は端末内のファイルから起動します。起動後はHTMLの更新確認をバックグラウンドで行います。

設定画面の「最新版を読み込む」は、安全バックアップを作成してからアプリキャッシュとService Workerを解除し、GitHub Pages上の最新版を再取得します。
...
- localStorage互換性を維持
- 既存ユーザーデータを破壊しない
- 起動速度改善では、既存データ形式の変更よりもアプリシェルキャッシュ、読み込み、集計、描画の負荷軽減を優先する
